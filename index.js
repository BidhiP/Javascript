...
(function () {
    ...
    const userList = (function () {
        ...
        // Array to store names from nameDisplay function
        const namesArray = [];

        const handleAddNewUserClick = (e) => {
            // Put your logic for putting list inside array
            users.push({ name: 'bidhi', phone: '3425' });
            // Assuming $username is a variable, you can push it to the namesArray
        ...
        const renderUsers = () => {
            // Render users from the 'users' array
            users.forEach((user) => {
                DOM.userListWrapperEl.innerHTML += `<li class="user-list__add-new-user">${user.name}</li>`;
            });

            // Render names from the 'namesArray'
            namesArray.forEach((name) => {
                DOM.userListWrapperEl.innerHTML += `<li class="user-list__add-new-user">${name}</li>`;
            });
        }
